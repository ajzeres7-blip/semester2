...
def insert_from_csv(filename):
     ...
     with open(filename, 'r') as fly:
          reader=csv.reader(fly)
          next(reader) #done to skip header
...
#querying with different filters
def query_contacts():
     conn=connect()
     cur=conn.cursor()
     ...
     if choice=="1":
          ...
          name==input("Enter name: ")
          cur.execute(
               "SELECT * FROM phonebook WHERE name ILIKE %s", #ILIKE-caseinsensitive
               ('%'+ name +'%',)
          )
     elif choice=="3":
          prefix=input("Enter prefix: ")
          cur.execute(
               "SELECT * FROM phonebook WHERE phone LIKE" #LIKE-case sensitive
               (prefix + "%",)
          )
...
# deleting a contact by username or phone number
def delete_contact():
     conn=connect()
     cur=conn.cursor()
     ...
     if choice=="1":
          ...
          phone=input("Enter phone: ")
          cur.execute("DELTE FROM phonebook WHERE phone=%s",(phone,))
